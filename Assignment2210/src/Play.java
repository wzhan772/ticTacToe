...
public class Play extends JFrame {
	...
	private int numUNAVAILABLEPositions; /* Number of UNAVAILABLE positions */
	...
	public Play(String fileName) {
		...
		try {
		    ...
		    t = new Evaluate(rowsBoard, adjacentToWin, maxLevel); /* Student code */

		    for (int row = 0; row < rowsBoard; ++row) {
			line = inFile.readLine();
			for (int col = 0; col < colsBoard; ++col)
			    if (line.charAt(col) == UNAVAILABLE) {
				gameDisplay[row][col].setIcon(new ImageIcon("unavailable.gif"));
				gameDisplay[row][col].paint(gameDisplay[row][col].getGraphics());
				t.storePlay(row, col, UNAVAILABLE);
			    }
		    }
		    inFile.close();
		}
			...
			System.out.println("Error in class Evaluate: "+e.getMessage());
			System.exit(0);
		}
	}

	/*
	 * To run the program type: java Play size to_win, where size is the size
	...
	public static void main(String[] args) {
		...
		if (args.length < 1) {
			System.out.println("Usage: java Play input_file");
			System.exit(0);
		}

		/* Create the game board and start the game */
		JFrame f = new Play(args[0]);

		f.setSize(((Play)f).colsBoard * 100, ((Play)f).rowsBoard * 100);
		f.setVisible(true);
		...
		if (Math.random() > 0.5) 
			((Play)f).handler.displayComputerPlay();
	}
				...
				for (int c = 0; c < boardCols; c++) {
					if (t.squareIsEmpty(row, column)) { // Empty position
						...
						if (t.wins(symbol) || t.isDraw() || (level >= maxLevel))
							...
							lookupVal = t.repeatedState(configurations);
							if (lookupVal != null)
								...
								if (t.repeatedState(configurations) == null)
								    t.insertState(configurations, reply.getScore(), reply.getLevel());
							}
						}
